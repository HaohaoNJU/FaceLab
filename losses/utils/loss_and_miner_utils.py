...
def get_all_pairs_indices(labels, ref_labels=None):
    ...
    # else:
    #     bs = len(labels)
    #     ref_labels = torch.cat([ref_labels[bs:],labels],dim=0)  # TODO: wh add

    labels1 = labels.unsqueeze(1) # n1 x 1
    labels2 = ref_labels.unsqueeze(0) # 1 x n2
    matches = (labels1 == labels2).byte() # n1 x n2
    diffs = matches ^ 1
    if ref_labels is labels:
        matches -= torch.eye(matches.size(0)).byte().to(labels.device)
    a1_idx = matches.nonzero()[:, 0].flatten() # index of labels1
    p_idx = matches.nonzero()[:, 1].flatten() # index of labels2
...
def convert_to_triplets(indices_tuple, labels, t_per_anchor=100):
    ...
    if indices_tuple is None:
        ...
        for i in range(len(labels)):
            # 对于当前batch里面的每一个embedding，找到所有以它为anchor的pos和neg embedding，补齐一下
            #  如：找到正对[（p1,a）,(p2,a)]，负对[(n1,a)]，补齐为[(p1,a,n1),(p2,a,n1)]
            pos_idx = (a1 == i).nonzero().flatten()
            neg_idx = (a2 == i).nonzero().flatten()
            if len(pos_idx) > 0 and len(neg_idx) > 0:
                p_idx = p[pos_idx]
                n_idx = n[neg_idx]
                p_idx, n_idx = matched_size_indices(p_idx, n_idx) # make their lengths to be the same
                # a_idx = torch.ones_like(c_f.longest_list([p_idx, n_idx])) * i
                a_idx = torch.ones(len(p_idx)).long() * i
                a_out.append(a_idx)
                p_out.append(p_idx)
                n_out.append(n_idx)
