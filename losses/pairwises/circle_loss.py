...
class CircleLoss(BaseMetricLossFunction):
    ...
    def compute_loss(self, embeddings, labels, indices_tuple):
        ...
        # for idx in range(len(anchor_idx)):
        #     print(anchor_idx[idx],positive_idx[idx],negative_idx[idx])
        similarity_matrix = embeddings.mm(embeddings.t())
        label_matrix = (labels.unsqueeze(1) == labels.unsqueeze(0)).float()
        #make eye_diongl tensor
        eye_diongal = torch.diag(label_matrix, diagonal=0)
        eye_diongal = torch.diag(eye_diongal, diagonal=0)
        positive_matrix = (label_matrix - eye_diongal).bool()
        negative_matrix = (label_matrix - 1).abs_().bool()
        ...
        for anchor in range(embeddings.size(0)):
            ...
            # sp_for_this_anchor = sp[mask]#duplicate
            # sn_for_this_anchor = sn[mask]#duplicate
            #print(len(sp_for_this_anchor),len(sn_for_this_anchor))
            sp_for_this_anchor = similarity_matrix[anchor][positive_matrix[anchor]]
            sn_for_this_anchor = similarity_matrix[anchor][negative_matrix[anchor]]
