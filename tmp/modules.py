...
class Header(nn.Module):
    ...
    def save(self,epoch=0,steps=0,save_weight_path=None):
        for (module,tag,_,_,_) in self.learnable_modules:
            save_dir = os.path.join(save_weight_path,"epoch{}_steps{}_{}.pth".format(epoch,steps,tag))
            if hasattr(module, "module"):
                torch.save(module.module.state_dict(), save_dir)
            else:
...
class IOFactory():
    def __init__(self, save_path, tag, vis=True, log_name="train"):
        """
            Make paths for all useful files and enables logging
        """
        self.tag = tag + "_" + str(datetime.datetime.today())[:10]
